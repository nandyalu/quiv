...
5. Expecting `shutdown()` to wait for `run_on_main` work — it does not. The handler returns as soon as it hands the work over, so the job is already complete and `shutdown()` finds nothing running; in FastAPI the loop then closes and cancels the work. quiv cannot wait (it is called from the loop's own thread, so blocking deadlocks). Work that must not be lost needs an owner on the main loop, such as an `asyncio.Queue` you drain in the lifespan after `shutdown()`.
