...
Work toward v1.0.0 followed a fixed, phased roadmap: `docs/roadmap.md` (what and why) and `plans/` (how — one detailed plan per phase with prescriptive design decisions, tests, pitfalls, and exit checklists; start at `plans/README.md`). Work after v1.0.0 continues the same way: Phase 7 (process jobs, v1.1.0) is planned in `plans/phase-7-process-jobs.md`. Every 1.x phase is additive — no 1.0 name or default changes; a breaking change waits for 2.0.0.
...
- **`shutdown()` does not wait for `run_on_main` work; `ashutdown()` does.** A handler that hands work to the main loop returns at once, so its job is already complete and `shutdown()` finds nothing running; the loop then closes and cancels the work. **Never "fix" this by blocking inside `shutdown()`** — it is synchronous and the lifespan calls it from the main loop's own thread, so a wait there blocks the very thread the work needs (measured: the full timeout, no progress). `ashutdown()` is the async counterpart and is the only safe place to wait, because awaiting yields that thread. It runs `shutdown()` in an executor first (keeping the loop free so a finishing job's handoffs still progress), then drains. `QuivBase._main_loop_work` is the tracked set, written by `run_on_main` across all four dispatch shapes; a **sync** callable sent with `call_soon_threadsafe` has no future, so a plain marker object stands in for it from the moment it is queued until it has run — without that, a drain sees an empty set while the callback is still in the ready queue. `_will_drain_main_loop_work` suppresses the warning while `ashutdown` is the caller. Three edges, each with a test: the drain must run on the loop that **owns** the work (a tracked `asyncio.Task` awaited from another loop raises `ValueError: The future belongs to a different loop`, so `ashutdown` marshals it with `run_coroutine_threadsafe`; a tracked `concurrent.futures.Future` is fine anywhere via `wrap_future`); `_resolve_main_loop()` returns `None` for a closed loop, so that case must **not** drain at all, or `timeout=None` hangs forever on work that can never progress; and `run_on_main` untracks its marker if `call_soon_threadsafe` raises, or the marker outlives the process. **With a `timeout`, abandoned jobs can hand work over after the drain ends and that is unfixable** — the thread cannot be stopped — so `ashutdown` warns instead of pretending the queue is closed.
