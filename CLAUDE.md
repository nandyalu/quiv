...
- **`shutdown()` deliberately does not wait for `run_on_main` work.** A handler that hands work to the main loop returns at once, so its job is already complete and `shutdown()` finds nothing running; the loop then closes and cancels the work. **Do not "fix" this by blocking in `shutdown()`** — it is synchronous and the documented lifespan pattern calls it from the main loop's own thread, so a wait there blocks the very thread the work needs (measured: the full timeout, no progress). The remedy belongs in the application, which is already on the loop and can await its own queue after `shutdown()` returns. Documented in `docs/run-on-main.md` and the `shutdown`/`run_on_main` docstrings.
