...
- **Waiters**: `wait_for_job`/`await_job` (by job id) and `wait_for_task`/`await_task` (the task's next job to finalize) return the finalized `Job`. The futures live in `_job_waiters`/`_task_waiters` under `_registries_lock`. A waiter registers first, then reads the row — register-then-read is the correctness argument: `_run_job` writes the terminal status, emits the `JOB_*` events, then pops and resolves the waiters, so a waiter registered before the pop is resolved by it and one registered after the pop sees the terminal status. Timeouts raise the builtin `TimeoutError` (3.10's separate `concurrent.futures`/`asyncio` classes are converted). `remove_task` fails a task's waiters with `TaskNotFoundError` unless a job of it is in flight — a `scheduled` or `running` job row, or a row that was `running` at deletion (`persistence.delete_task` returns the status it had; delete and `mark_task_running` share `_write_lock`, so that status is exact) — and `_dispatch_due_task` fails them itself when it finds the row gone after marking it, so no waiter is left for a job that never comes. `shutdown()` fails every leftover waiter with `SchedulerStoppedError` after the pool drain, and `_add_waiter` checks `_shutdown` under `_registries_lock` so it cannot register a future after that clear. Every `set_result`/`set_exception` is guarded by `fut.done()`, because a timed-out or cancelled waiter is already done.
- **Job context**: `_run_job` sets `_current_quiv` and `_current_job_id` (both in `context.py`); `_current_stop_event()` reads both and never the process-level fallback. Same reach as the active instance: nested sync calls, the worker event loops, `asyncio.create_task`, `asyncio.to_thread`; not a `threading.Thread` the handler starts.
- **Helpers and cancellation**: `call_on_main()` (the waiting sibling of `run_on_main`; no options of its own, every keyword forwards; polls the future in 0.1 s slices and cancels it when the stop event is set; an async target from the loop's own thread raises `MainLoopUnavailableError`) and `run_subprocess()` (`quiv/subprocesses.py`; `communicate()` in 0.1 s slices; on stop `terminate()`, `kill()` after `kill_grace`, always `wait()` after `kill()`; `subprocess.TimeoutExpired` on timeout, stdlib semantics) both raise `JobCancelledError` when the stop event fires while they wait. `_run_job` treats a `JobCancelledError` raised while `_stop_requested(job_id)` is true as a clean stop: one info line, no traceback, `error_message` None (the timeout text still leads when a timeout set the event). Raised with no stop requested, it is an ordinary failure.
