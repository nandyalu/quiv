...
Work toward v1.0.0 followed a fixed, phased roadmap: `docs/roadmap.md` (what and why) and `plans/` (how — one detailed plan per phase with prescriptive design decisions, tests, pitfalls, and exit checklists; start at `plans/README.md`). Work after v1.0.0 continues the same way: Phase 7 (process jobs, v1.2.0) is planned in `plans/phase-7-process-jobs.md`; v1.1.0 shipped outside the phases (`update_task(run_at=)`, `pending_main_loop_work()`), so the phase numbers and the version numbers no longer line up. Phases 8 (waiting on work: job/task waiters, `call_on_main()`, `run_subprocess()`; `plans/phase-8-waiting-on-work.md`) and 9 (operations: `is_running`, `after_current`, `task_name` filter, the container page; `plans/phase-9-operations.md`) came out of the 2026-09-25 review of trailarr and ten-acre; they do not depend on Phase 7 and may ship before it, with versions fixed at release. Every 1.x phase is additive — no 1.0 name or default changes; a breaking change waits for 2.0.0.
