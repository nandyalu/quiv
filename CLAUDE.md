...
- **Event listeners**: Global `add_listener(Event, callback)` / `remove_listener(Event, callback)` on `QuivBase`. Listeners are stored in `_event_listeners: dict[Event, list[Callable]]`. `_emit_event()` dispatches to the main loop using the same pattern as progress callbacks (async via `run_coroutine_threadsafe`, sync via `call_soon_threadsafe`, fallback to direct call). Exceptions in listeners are logged and swallowed. `TASK_*` callbacks receive `(event, task: Task)`, `JOB_*` callbacks receive `(event, task: Task, job: Job)`. The `Job` model includes `duration_seconds` and `error_message` fields set during finalization. Events: `TASK_ADDED`, `TASK_REMOVED`, `TASK_PAUSED`, `TASK_RESUMED`, `JOB_STARTED`, `JOB_COMPLETED`, `JOB_FAILED`, `JOB_CANCELLED`, `JOB_RETRYING` (fires after `JOB_FAILED` when a retry was scheduled).
