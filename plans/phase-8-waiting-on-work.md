...
- A waiter returns the finalized `Job`, with `status`, `duration_seconds`, and `error_message`, after quiv wrote it and after the `JOB_*` event for it was emitted. A listener sees a job before a waiter does.
