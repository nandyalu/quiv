...
- `JobCancelledError` is the one exception both helpers raise when the stop event fires while they wait. The handler lets it propagate; `_run_job` already turns "stop event set" into `CANCELLED` whatever the handler raised. **Found in implementation:** `_run_job` logged that unwind as an error with a traceback and stored the exception text as `error_message`, the same noise the 1.0.1 fix removed from `run_on_main`. So `_run_job` now treats a `JobCancelledError` raised while the job's stop event is set as a clean stop: one info line, no traceback, `error_message` stays `None` (or the timeout text when a timeout set the event). A `JobCancelledError` raised with no stop requested is an ordinary exception and the job fails. `SchedulerStoppedError` is what a waiter gets when `shutdown()` returns before the job finished, and what the wait methods raise after shutdown.
...
- **Found in review (Copilot on PR #83).** Three real ones. `_add_waiter` checked `_shutdown` outside the lock, so a waiter could register after `shutdown()` cleared the tables; the check now sits under `_registries_lock`. `remove_task` scanned `RUNNING` jobs only, so a dispatch between `mark_task_running` and `create_job` was invisible: its waiters were failed and the job then ran uncancelled; now `delete_task` returns the status the row had (exact, both take `_write_lock`), `SCHEDULED` jobs count as in flight and get their stop event, and `_dispatch_due_task` fails the waiters itself when the row vanished after it was marked. `run_subprocess` forwarded `input=` to `Popen`, which rejects it; it is now an explicit parameter passed to the first `communicate()` only.
