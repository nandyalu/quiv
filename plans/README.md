...
1. **Do the phases in order.** Later phases assume earlier machinery (e.g. Phase 4 timeout enforcement rides on Phase 2's wake-event loop).
   ...
   **Phases 8 and 9 do not build on Phase 7, and they ship before it.** They came out of the 2026-09-25 review of the two applications that run quiv, and both applications need them sooner than process jobs. The order decided on 2026-09-25 is Phase 8 (`v1.2.0`), Phase 9 (`v1.3.0`), then Phase 7 (`v1.4.0`). Phase 7's §0, the three-platform CI matrix, is pulled forward and lands before Phase 8, because Phase 8 holds the platform-sensitive code. The phase numbers never change; only the version column does.
